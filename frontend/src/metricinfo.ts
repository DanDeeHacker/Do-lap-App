...
export const METRIC_INFO: Record<string, string> = {
  ...
  readiness:
    "Připravenost (20–100 %) = jak daleko jsou dnes vaše ukazatele regenerace od VAŠÍ normy (posledních 8 týdnů): HRV pod normou a klidový tep nad normou — z poslední noci i z průměru 7 nocí, který je spolehlivější a počítá se o čtvrtinu silněji (to, co hodinky ukazují jako stav HRV) — kratší nebo méně kvalitní spánek (méně hlubokého a REM spánku nebo nižší efektivita než obvykle — počítá se nejvýš napůl, protože hodinky fáze spánku jen odhadují) a svalová bolest / únava z check-inu. Malá odchylka v mezích vašeho běžného kolísání mezi dny je šum a nic nestojí; čím větší odchylka, tím nižší připravenost. Shodné signály se sčítají: když jsou HRV i klidový tep zhruba o jedno vaše běžné kolísání mimo normu, vychází kolem 70 %, když skoro o dvě, kolem 40 %, nejníž 20 %. Kapacitu (Zátěž) připravenost zmenšuje nejvýš o 30 %.",
  sleepQuality:
  ...
  readinessTraining:
    "Připravenost (20–100 %) = odchylka HRV, klidového tepu (poslední noc i průměr 7 nocí), spánku a check-inu od vaší normy za 8 týdnů. HRV i klidový tep zhruba o jedno vaše běžné kolísání mimo normu → kolem 70 %, skoro o dvě → kolem 40 %. Pod 65 % dnes bez tvrdého tréninku a dlouhého běhu, pod 45 % jen regenerace; zmenšuje i dnešní strop na jeden běh. Týdenní cíl z cyklu nemění.",
}
